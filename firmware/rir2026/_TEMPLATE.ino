 ...
 *  ETIQUETA: {{ETIQUETA}}
 * ============================================================================
 *  Cluster:  {{CLUSTER_ID}}   (cluster_id no POST)
 *  Seccao:   {{SECAO}}        ("m" | "f" | "u")
 *  Porta:    {{PORTA}}        ("LL" | "LR" | "C" | "")
 *  USAR_IR:  {{USAR_IR}}      (true = LLH/LRH/LLW/LRW; false = LC/unissexo)
 *  Cap:      {{CAPACIDADE}}
 *  Coord:    {{LAT}} / {{LON}}
 * ============================================================================
...
const char* NOME_CURTO = "{{NOME_CURTO}}";
const char* PORTA_ID   = "{{PORTA}}";    // "LL", "LR", "C", ""
const char* SECAO_ID   = "{{SECAO}}";   // "m", "f", "u"
...
void setup() {
  ...
  Serial.printf("Cluster: %s  Cap: %d  IR: %s\n",
    CLUSTER_ID, CAPACIDADE, {{USAR_IR_STR}});
  bootMs = millis();
