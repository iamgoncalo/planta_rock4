...
def dijkstra_min(origem: str) -> dict[str, float]:
    ...
    return {n: round(d / max(VELOCIDADE_M_MIN, _EPS), 1)
            for n, d in dist.items() if n in _GEO}
...
def compute_route(origem: str, genero: str,
    ...
    key = f"{origem.upper()}|{(genero or 'f').lower()[:1]}"
