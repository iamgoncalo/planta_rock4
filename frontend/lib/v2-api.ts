...
// ════════════════════════════════════════════════════════════════════
// v8 · OPERACIONAL — cleaning · staff · incidents · weather · forecast
// ════════════════════════════════════════════════════════════════════

export type CleaningStatus = 'clean' | 'in_progress' | 'needs_cleaning' | 'urgent';

export interface CleaningClusterStatus {
  cluster_id: string;
  status: CleaningStatus;
  last_cleaned_at: string | null;
  next_scheduled_for: string | null;
  last_team: string | null;
  minutes_since_clean: number | null;
}

export interface CleaningScheduleResponse {
  clusters: CleaningClusterStatus[];
  generated_at: string;
}

export interface CleaningDoneRequest {
  cluster_id: string;
  team?: string | null;
...
export type StaffRole =
  | 'cleaning' | 'steward' | 'medic' | 'security' | 'supervisor';
...
export interface StaffRosterResponse {
  day: string;
  total: number;
  by_cluster: Record<string, StaffEntry[]>;
...
export interface ForecastResponse {
  cluster_id: string;
  horizon_min: number;
  current_ocupacao_pct: number;
  points: ForecastPoint[];
  peak_ocupacao_pct: number;
  peak_at_min: number;
  notes: string[];
  generated_at: string;
}
...
// Anexa as funções operacionais ao objecto `api` já exportado
const _opsExtension = {
  cleaningSchedule: () =>
    getJson<CleaningScheduleResponse>('/api/v1/cleaning/schedule'),

  cleaningMarkDone: (body: CleaningDoneRequest) =>
    postJson<unknown>('/api/v1/cleaning/done', body),
  ...
  incidentsList: (
    ...
    return getJson<IncidentListResponse>(
      `/api/v1/incidents/${q ? `?${q}` : ''}`,
    );
  },

  incidentCreate: (body: IncidentCreateRequest) =>
    postJson<IncidentEntry>('/api/v1/incidents/', body),

  incidentResolve: (id: number, body: IncidentResolveRequest) =>
    postJson<IncidentEntry>(`/api/v1/incidents/${id}/resolve`, body),

  staffRoster: (day: string) =>
    getJson<StaffRosterResponse>(`/api/v1/staff/roster?day=${day}`),

  weatherNow: () => getJson<WeatherNow>('/api/v1/weather/now'),

  forecastCluster: (clusterId: string, horizonMin = 30) =>
    getJson<ForecastResponse>(
      `/api/v1/forecast/cluster/${clusterId}?horizon_min=${horizonMin}`,
    ),
};
...
// ════════════════════════════════════════════════════════════════════
// v9 · 14 unidades operacionais + SCOR observability + Pipelines
// ════════════════════════════════════════════════════════════════════
...
export interface CleaningV9Response {
  units: UnitStatus[];
  total_units: number;
  clean_count: number;
  needs_count: number;
...
export interface ScorRecentResponse { records: ScorRecord[]; count: number; }

export interface ScorStats {
  ...
  avg_latency_ms_5min: number;
}

export interface ScorOverview {
  stats: ScorStats;
  latest: ScorRecord | null;
  config: { url: string; interval_s: number; dry_run: boolean };
  generated_at: string;
}
...
const _opsExtensionV9 = {
  cleaningUnitsCatalog: () => getJson<unknown>('/api/v1/cleaning/units'),
  cleaningUnitsStatus: () => getJson<CleaningV9Response>('/api/v1/cleaning/units/status'),
  cleaningUnitDone: (unit_id: string, body: { team?: string | null; operator?: string | null; notes?: string | null }) =>
    postJson<CleaningHistoryEntry>(`/api/v1/cleaning/units/${unit_id}/done`, body),
  cleaningUnitHistory: (unit_id: string, limit = 50) =>
    getJson<CleaningHistoryEntry[]>(`/api/v1/cleaning/units/${unit_id}/history?limit=${limit}`),
  cleaningHistoryRecent: (limit = 50) =>
    getJson<CleaningHistoryEntry[]>(`/api/v1/cleaning/history?limit=${limit}`),
  scorRecent: (limit = 50) => getJson<ScorRecentResponse>(`/api/v1/scor/recent?limit=${limit}`),
...
// Tipo combinado: api original + _opsExtension (v8) + _opsExtensionV9 (v9)
type _ApiV9Extended = typeof api & typeof _opsExtension & typeof _opsExtensionV9;
export const apiV9 = api as _ApiV9Extended;
